...
module herringboneGear(
	...
	if(generate != 2)
	{
		translate([-g1p_r,0,0]) gearsbyteethanddistance(
				distance_between_axels = 60, 
				gear_h = 15, 
				gear_shaft_h = 15,
				gear1_teeth = 13,
	...
	if(generate != 1)
	{
		translate([g2p_r,0,0]) gearsbyteethanddistance(
			distance_between_axels = 60, 
			gear_h = 15, 
			gear_shaft_h = 15,
			gear1_teeth = 13,
...
module gearsbyteethanddistance(t1=13,t2=51, d=60, teethtwist=1, which=1, gear_h, TT)
{
	cp = 360*d/(t1+t2);
	...
	else
	{
		...
		translate([0,0,gear_h+gear_shaft_h-gear2_bolt_sink])
			bridge_helper(bridge_helper_h, gear2_bolt_hex_r, TT, gear2_bolt_hex_d, ST, gear2_shaft_d, AT);
		translate([0,0,gear_h+gear_shaft_h-gear2_bolt_sink+bridge_helper_h/3])
			rotate([0,0,60]) bridge_helper(bridge_helper_h, gear2_bolt_hex_r, TT, gear2_bolt_hex_d, ST, gear2_shaft_d, AT);
